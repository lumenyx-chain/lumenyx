...
# Run as validator
./target/release/lumenyx-node --chain mainnet --validator
```
...
Use Polkadot.js Apps connected to `ws://localhost:9944`:
1. Call `validatorFaucet.claimForValidator(account, nonce, pow_solution)`
2. Requires proof-of-work (~2 seconds to compute)
3. One claim per account
...
```bash
./target/release/lumenyx-node --chain mainnet --validator --name "your-name"
```
