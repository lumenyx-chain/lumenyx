...
use sp_runtime::traits::{Block as BlockT, Header as HeaderT, SaturatedConversion, NumberFor};

use sc_client_api::{AuxStore, HeaderBackend, StorageProvider};
use sc_consensus::{BlockCheckParams, BlockImport, BlockImportParams, ForkChoiceStrategy, ImportResult};
use sp_consensus::SelectChain;

use super::{
	Block, FullBackend,
	LUMENYX_ENGINE_ID, RxLxPow,
	read_difficulty_from_storage, difficulty_to_target, hash_meets_target,
};
...
fn ensure_single_seal_in_post_digests(params: &mut BlockImportParams<Block>, nonce: [u8; 32]) {
	params.post_digests.retain(|d| !matches!(d, DigestItem::Seal(id, _) if *id == LUMENYX_ENGINE_ID));
	params.post_digests.push(DigestItem::Seal(LUMENYX_ENGINE_ID, nonce.to_vec()));
}
...
impl<C> LumenyxPowBlockImport<C> {
		...
		C: HeaderBackend<Block> + StorageProvider<Block, FullBackend>,
	{
		...
		let Some(pow) = guard.as_mut() else { return false; };
	...
	C: HeaderBackend<Block> + AuxStore + Send + Sync + 'static,
{
	...
	async fn best_chain(&self) -> Result<<Block as BlockT>::Header, sp_consensus::Error> {
		...
		let (best_hash, _) = best.ok_or(sp_consensus::Error::StateUnavailable("no leaves".into()))?;
		let header = self.client.header(best_hash)
			.map_err(|_| sp_consensus::Error::ClientImport("header error".into()))?
			.ok_or(sp_consensus::Error::StateUnavailable("header not found".into()))?;
